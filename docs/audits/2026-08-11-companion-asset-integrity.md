...
The current companion documentation declares paths such as:
...
Repository-path validation on the current branch returned `404` for the sampled generic, hero and exclusive files. Those paths must not remain embedded in the root README until real binaries exist.
