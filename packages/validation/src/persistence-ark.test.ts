...
test("Memory Receipt Ark migration preserves receipt and action-id authority", () => {
  ...
  assert.match(migration, /receipt_id TEXT NOT NULL UNIQUE REFERENCES memory_receipts\(id\)/i);
...
test("PERN remains contract-ready until a live driver and transaction-bound mutation are proven", () => {
  ...
  assert.match(foundation, /concrete governed PostgreSQL driver\/pool/i);
  assert.match(roadmap, /Remaining before PostgreSQL becomes `active`/i);
